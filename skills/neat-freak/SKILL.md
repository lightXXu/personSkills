...
**反幻觉审查（再查这组，防止把纠偏痕迹写进文档）**：
- [ ] 没有因为前面误判/幻觉了 A，后来又在 docs 或规则里写"不要 A"、"不再 A"、"不会 A"，让一个本不该出现的概念进入读者视野
- [ ] 否定性表述都指向真实存在且读者必须知道的边界（如不要提交 token），不是为了修正本轮对话里的误解
- [ ] 新增文档优先使用正向模型说明（"使用 X/Y/Z"），避免保留旧字段、旧文件、旧配置、旧流程的名字，除非读者迁移时必须知道
- [ ] 对 grep 命中的 `不再|不会|不要|不建议|暂不|do not|does not|no longer|fallback|deprecated` 逐条判断：是必要边界就保留，是历史纠偏就删除或改成正向说明
